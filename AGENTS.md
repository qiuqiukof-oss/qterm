<!-- CLI-Q context start -->
# CLI-Q 运行上下文（由 CLI-Q 自动生成，请勿手动修改本段）

你正在 **CLI-Q** 中运行 —— 这是一个「浏览器里的终端 + AI 智能体中枢」。
CLI-Q 的 Web 控制台地址是 http://127.0.0.1:4264，服务端口为 **4264**。

## 你能使用的 MCP 工具
CLI-Q 已在 `.mcp.json` 中注册了名为 **`cli-q`** 的 MCP 服务器，提供两大类工具：
...
- **会话 / 终端**：`session_create` / `session_write` / `session_read` / `session_list`
  等，用于操作 CLI-Q 托管的持久终端会话。

## 使用浏览器自动化的前提
要使用上述浏览器工具，需先有一个开启了远程调试端口的浏览器实例：
- 端口固定为 **9222**（`http://127.0.0.1:9222`）。
- 最简单方式：在 CLI-Q 系统托盘菜单里选择「打开（CDP 模式）」，它会用独立
...
- 默认浏览器上下文（context 0）是 CLI-Q 管理页面本身，**对其进行导航/点击会导致
